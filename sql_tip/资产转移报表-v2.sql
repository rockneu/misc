SELECT TO_CHAR(TH.TRANSACTION_DATE_ENTERED,'RRRRMM')
         ||LPAD(DENSE_RANK() over(partition by TO_CHAR(TH.TRANSACTION_DATE_ENTERED, 'RRRR-MM') order by TH.TRANSACTION_HEADER_ID), 5, '0') 处置序列号,
       B.ASSET_NUMBER 资产编号,
